...
mod tests {
    ...
    fn score_vertices_check_count() {
        ...
        assert_eq!(problem.score_vertices_check_count(&vec![], &None), Err(PoseValidationError::VerticeCountMismatch));
        assert_eq!(problem.score_vertices_check_count(&vec![Point(15,21), Point(34,0), Point(0,45)], &None), Err(PoseValidationError::VerticeCountMismatch));
        assert_eq!(problem.score_vertices_check_count(&vec![Point(15,21), Point(34,0), Point(0,45), Point(19,24)],  &None), Ok(()));
        assert_eq!(problem.score_vertices_check_count(&vec![Point(15,21), Point(34,0), Point(0,45), Point(0,45), Point(0,45)], &None), Err(PoseValidationError::VerticeCountMismatch));
    }

    #[test]
    fn score_vertices_check_stretching() {
        let problem: Problem = serde_json::from_str(PROBLEM_13_JSON).unwrap();
        assert_eq!(problem.score_vertices_check_stretching(&vec![Point(15,21), Point(34,0), Point(0,45), Point(19,24)], &None), Ok(()));
